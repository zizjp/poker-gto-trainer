/**
 * poker-gto-trainer - Main App (v0.8.2)
 * 苦手問題復習機能追加
 */
...
function App() {
  ...
  const handleReviewMode = () => {
    ...
    // 誤答したハンドのみを抽出
    const incorrectQuestions = trainingResult.questions.filter(q => !q.isCorrect);
    
    if (incorrectQuestions.length === 0) return;
    
    // 復習用の問題リストを作成（userAnswerをクリア）
    const reviewQuestions: TrainingQuestion[] = incorrectQuestions.map(q => ({
      hand: q.hand,
      correctActions: q.correctActions,
      // userAnswerとisCorrectをクリアして再挑戦
    }));
    
    setTrainingQuestions(reviewQuestions);
    
    // 頻度表示モードの場合、トラッカーを再初期化
    if (trainingSettings.judgmentMode === 'frequency') {
      const tracker = initializeFrequencyTracker(reviewQuestions);
      setFrequencyTracker(tracker);
    } else {
      setFrequencyTracker(null);
    }
  ...
  // 苦手問題復習モードを開始
  const handleStartWeakHandsTraining = (weakHands: string[]) => {
    // 統計画面を閉じる
    ...
    // 弱点ハンドが所属するレンジを探す
    const rangesWithWeakHands = savedRanges.filter(range => {
      return weakHands.some(hand => hand in range.hands);
    });
    
    if (rangesWithWeakHands.length === 0) return;
    
    // 最初に見つかったレンジを使用
    const targetRange = rangesWithWeakHands[0];
    
    // 弱点ハンドのみで問題を生成
    const weakHandQuestions: TrainingQuestion[] = weakHands
      .filter(hand => hand in targetRange.hands)
      .map(hand => ({
        hand: hand as Hand,
        correctActions: targetRange.hands[hand as Hand],
      }));
    
    if (weakHandQuestions.length === 0) return;
    
    // トレーニング設定を作成
    const settings: TrainingSettings = {
      rangeId: targetRange.id,
      rangeName: `${targetRange.name} - 苦手問題復習`,
      coverage: 100,
      judgmentMode: 'probabilistic',
      totalQuestions: weakHandQuestions.length,
    };
    
    setTrainingSettings(settings);
    setTrainingQuestions(weakHandQuestions);
    setFrequencyTracker(null);
    setTrainingPhase('session');
  };
    ...
    <div className={`app min-h-screen ${appMode === 'debug' ? 'bg-gray-50' : 'bg-green-50'}`}>
      ...
      {showStatistics && (
        <Statistics 
          ...
          onStartWeakHandsTraining={handleStartWeakHandsTraining}
        />
      )}
        ...
        <>
          ...
          <header className="mb-8">
            ...
            <p className="text-gray-600 mt-2">
              ...
              {/* 学習モードのコンテンツ */}
              {appMode === 'training' && savedRanges.length === 0 && (
              ...
              <footer className="mt-12 text-center text-sm text-gray-500">
                <p>{appMode === 'debug' ? 'Phase 1: カスタムレンジエディター' : 'Phase 3: 学習モード（v0.8.2）'}</p>
          ...
          {trainingPhase === 'setup' && !showEditor && appMode === 'training' && savedRanges.length > 0 && (
            // 学習設定画面（単独表示）
            <TrainingSetup
              ranges={savedRanges}
